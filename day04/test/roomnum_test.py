...
def test_four():
    ...
    assert r.isValid(), "Wrong: %s, %s, %s" % (r.isValid(), str(r.countMap), str(r.checkSum))
...
def test_decrypt():
    r = RoomNum('qzmt-zixmtkozy-ivhz-343[check]')
    assert r.decrypt() == 'very encrypted name', "Was: '%s'" % r.decrypt()
